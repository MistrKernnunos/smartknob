...
// Run calibration once at startup, then update these constants with the
// calibration results.
static const float ZERO_ELECTRICAL_OFFSET = 4.59;
...
MotorTask::run()
{
  ...
  // Not actually using the velocity loop built into SimpleFOC; but I'm using
  // those PID variables to run PID for torque (and SimpleFOC studio supports
  // updating them easily over serial for tuning)
  ...
  while (1) {
      ...
      switch (command.command_type) {
        ...
        case CommandType::CONFIG: {
          ...
          if (command.data.config.detent_strength_unit < 0) {
            log("Ignoring invalid config: detent_strength_unit cannot be "
                "negative");
            break;
          }
          if (command.data.config.endstop_strength_unit < 0) {
            log("Ignoring invalid config: endstop_strength_unit cannot be "
                "negative");
            break;
          }
          if (command.data.config.snap_point < 0.5) {
            log("Ignoring invalid config: snap_point must be >= 0.5 for "
                "stability");
            break;
          }
          if (command.data.config.detent_positions_count >
              COUNT_OF(command.data.config.detent_positions)) {
            log("Ignoring invalid config: detent_positions_count is too large");
            break;
          }
          if (command.data.config.snap_point_bias < 0) {
            log("Ignoring invalid config: snap_point_bias cannot be negative "
                "or there is risk of instability");
            break;
          }
          ...
          if (newConfig.position == INT32_MIN) {
            // INT32_MIN indicates no change to position, so restore from
            // latest_config
          ...
          if (newConfig.position != config.position ||
            ...
            // Only adjust the detent center if the position or width has
            // changed
          ...
          // When there are intermittent detents (set via detent_positions),
          // disable derivative factor as this adds extra "clicks" when nearing
          // a detent.
          motor.PID_velocity.D =
            config.detent_positions_count > 0
              ? 0
              : CLAMP(
                  raw,
    ...
    // Check where we are relative to the current nearest detent; update our
    // position if we've moved far enough to snap to another detent
    ...
    float snap_point_radians_decrease =
      snap_point_radians +
      (config.position <= 0 ? bias_radians : -bias_radians);
    ...
    if (angle_to_detent_center > snap_point_radians_decrease &&
        (num_positions <= 0 || config.position > config.min_position)) {
      ...
      config.position--;
               ...
               (num_positions <= 0 || config.position < config.max_position)) {
      current_detent_center -= config.position_width_radians;
      angle_to_detent_center += config.position_width_radians;
      config.position++;
    }
    ...
    bool out_of_bounds =
      num_positions > 0 &&
      ((angle_to_detent_center > 0 && config.position == config.min_position) ||
       (angle_to_detent_center < 0 && config.position == config.max_position));
    ...
    // strength, etc is handled by the PID_velocity parameters)
...
void
MotorTask::setConfig(const PB_SmartKnobConfig& config)
{
...
void
MotorTask::playHaptic(bool press)
{
  Command command = {
...
void
MotorTask::runCalibration()
{
  Command command = { .command_type = CommandType::CALIBRATE,
                      .data = {
                        .unused = 0,
                      } };
  xQueueSend(queue_, &command, portMAX_DELAY);
...
void
MotorTask::addListener(QueueHandle_t queue)
{
  listeners_.push_back(queue);
}

void
MotorTask::publish(const PB_SmartKnobState& state)
{
...
MotorTask::calibrate()
{
  ...
  snprintf(buf_,
           sizeof(buf_),
           "Going to measure %d electrical revolutions...",
  ...
  for (; a < destination1; a += 0.4) {
    ...
    snprintf(buf_,
             sizeof(buf_),
             "%.2f, %.2f, %.2f",
             degrees(real_electrical_angle),
             degrees(measured_electrical_angle),
...
MotorTask::checkSensorError()
{
  ...
  if (error.error) {
    snprintf(buf_,
             sizeof(buf_),
             "CRC error. Received %d; calculated %d",
             error.received_crc,
             error.calculated_crc);
    log(buf_);
  }
#endif
}
...
void
MotorTask::setLogger(Logger* logger)
{
  logger_ = logger;
}

void
MotorTask::log(const char* msg)
{
  if (logger_ != nullptr) {
    logger_->log(msg);
  }
